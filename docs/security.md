...
- **בקרות:**
  ...
  - **מפקד מבצע (`OPERATION_COMMANDER`)** הוא תפקיד קריאה בלבד: כל פונקציות הצפייה
    מחזירות לו `true`, וכל פונקציות הכתיבה והביצוע (`canEditPackageContent`,
    `canExecuteTask`, `canExecuteMission`, `canHandleAtHub`) מחזירות `false` במפורש.
    נתיב הכתיבה היחיד שנפתח לו הוא אישור או דחייה של בקשת הצטרפות לשליחות.
    כל פונקציה מטפלת בתפקיד במפורש ולא דרך ברירת מחדל, כדי שתפקיד חדש לא יקבל
    בשקט הרשאה שלא התכוונו לתת לו. מכוסה בבדיקות יחידה ובבדיקת אינטגרציה
    (403 לכל תפקיד אחר על `/dashboard/operation`).
...
- **בקרות** (`apps/api/src/app.module.ts`, `@nestjs/throttler`):
  ...
  - הערכים מרוכזים ב-`apps/api/src/common/http/rate-limits.ts`. ניתן לעקוף את מגבלת
    ההתחברות דרך `AUTH_RATE_LIMIT` לצורך חבילת הבדיקות בלבד; ברירת המחדל היא ערך
    הפרודקשן, וכל ערך לא תקין (לא מספר שלם חיובי) חוזר אליה - כלומר תקלת תצורה
    מקשיחה את הבקרה ולא מרפה אותה.
